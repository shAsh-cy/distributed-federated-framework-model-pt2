...
def load_femnist(
    ...
    """Load FEMNIST with its natural by-writer partition.
    ...
    Returns:
        ``(train, test, shards)`` — shards are contiguous index ranges into
        ``train``, one per selected writer, disjoint and exhaustive by
        construction.
    """
    cache_path = Path(cache_path)
